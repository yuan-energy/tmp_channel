...
//#                                   const and & issues                         #
//#                    August 30-31 '94 added use_def_dim to fool the CC         #
...
nDarray::nDarray(int rank_of_nDarray, double initval)
{
    ...
    // out 19Nov2007   int one_or0 = 0;
    // out 19Nov2007   if(!pc_nDarray_rep->nDarray_rank) one_or0 = 1;
    // out 19Nov2007
    // out 19Nov2007
    // out 19Nov2007
    // out 19Nov2007    pc_nDarray_rep->dim = new int[pc_nDarray_rep->nDarray_rank+one_or0];// array for
    // out 19Nov2007                                                                        // dimensions
    //     NEW 19Nov2007
    pc_nDarray_rep->dim = new int[4];// array for dimensions

    // new 19Nov2007
    // trying this new approach for use with optimized operator val,
    // this way all the tensors have 4 dimensions and they are set to have
    // depth of 1 by default
    // moved to header file 19Nov2007   pc_nDarray_rep->dim[4] = {1,1,1,1};
    pc_nDarray_rep->dim[0] = 1;
    pc_nDarray_rep->dim[1] = 1;
    pc_nDarray_rep->dim[2] = 1;
    pc_nDarray_rep->dim[3] = 1;
...
nDarray::nDarray(int rank_of_nDarray, int rows, int cols, double *values)
{
    ...
    // out 19Nov2007// not needed for BJmatrix or BJvector but who knows #
    // out 19Nov2007// in the case of nDarray_rank=0 add one to get right thing from the
    // out 19Nov2007// operator new
    // out 19Nov2007  int one_or0 = 0;
    // out 19Nov2007  if(!pc_nDarray_rep->nDarray_rank) one_or0 = 1;
...
void nDarray::Initialize_all( const nDarray &from )
{
    ...
    // out 19Nov2007// operator new
    // out 19Nov2007   int one_or0 = 0;
    // out 19Nov2007   if(!this->pc_nDarray_rep->nDarray_rank) one_or0 = 1;
    ...
    // out 19Nov2007
    // out 19Nov2007   this->pc_nDarray_rep->dim = new int[pc_nDarray_rep->nDarray_rank+one_or0];// array for
    // out 19Nov2007                                                                 // dimensions
    //     NEW 19Nov2007
    pc_nDarray_rep->dim = new int[4];// array for dimensions
...
nDarray nDarray::deep_copy()
{
    ...
    // out 19Nov2007// operator new
    // out 19Nov2007   int one_or0 = 0;
    // out 19Nov2007   if(!temp.pc_nDarray_rep->nDarray_rank) one_or0 = 1;
    delete [] temp.pc_nDarray_rep->dim; //delete default value

    // out 19Nov2007
    // out 19Nov2007   temp.pc_nDarray_rep->dim = new int[temp.pc_nDarray_rep->nDarray_rank+one_or0];// array for
    // out 19Nov2007                                                                 // dimensions
    ...
    // depth of 1 by default
    // moved to header file 19Nov2007   pc_nDarray_rep->dim[4] = {1,1,1,1};
    temp.pc_nDarray_rep->dim[0] = this->pc_nDarray_rep->dim[0];
    temp.pc_nDarray_rep->dim[1] = this->pc_nDarray_rep->dim[1];
    temp.pc_nDarray_rep->dim[2] = this->pc_nDarray_rep->dim[2];
    temp.pc_nDarray_rep->dim[3] = this->pc_nDarray_rep->dim[3];
...
// INLINE FUNCTION BODIES moved to header file #ifdef Brzi_nDarray_val
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file   // ..JB..//##############################################################################
// INLINE FUNCTION BODIES moved to header file   // ..JB..// another overloading of operator() . . .  // overloaded for ONE argument
// INLINE FUNCTION BODIES moved to header file   inline double & nDarray::val(int first)
// INLINE FUNCTION BODIES moved to header file     {
// INLINE FUNCTION BODIES moved to header file //      ::printf("\n KORISTIM   Brzi_nDarray_val \n ");
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file       long int where = first - 1;
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file   //::printf(" w=%ld ",where);
// INLINE FUNCTION BODIES moved to header file       double *p_value = pc_nDarray_rep->pd_nDdata + (size_t)where;
// INLINE FUNCTION BODIES moved to header file       return (*p_value);
// INLINE FUNCTION BODIES moved to header file     }
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file   //##############################################################################
// INLINE FUNCTION BODIES moved to header file   // another overloading of operator() . . .  // overloaded for TWO arguments
// INLINE FUNCTION BODIES moved to header file   inline double & nDarray::val(int first, int second)
// INLINE FUNCTION BODIES moved to header file     {
// INLINE FUNCTION BODIES moved to header file //      ::printf("\n KORISTIM   Brzi_nDarray_val \n ");
// INLINE FUNCTION BODIES moved to header file       long int where = first - 1;
// INLINE FUNCTION BODIES moved to header file                where = where*pc_nDarray_rep->dim[1]+second - 1;
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file   //::printf(" w=%ld ",where);
// INLINE FUNCTION BODIES moved to header file       double *p_value = pc_nDarray_rep->pd_nDdata + (size_t)where;
// INLINE FUNCTION BODIES moved to header file       return (*p_value);
...
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file   //##############################################################################
// INLINE FUNCTION BODIES moved to header file   // another overloading of operator() . . .  // overloaded for THREE arguments
// INLINE FUNCTION BODIES moved to header file   inline double & nDarray::val(int first, int second, int third )
// INLINE FUNCTION BODIES moved to header file     {
// INLINE FUNCTION BODIES moved to header file //      ::printf("\n KORISTIM   Brzi_nDarray_val \n ");
// INLINE FUNCTION BODIES moved to header file       long int where = first - 1;
// INLINE FUNCTION BODIES moved to header file                where = where*pc_nDarray_rep->dim[1]+second - 1;
// INLINE FUNCTION BODIES moved to header file                where = where*pc_nDarray_rep->dim[2]+third  - 1;
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file   //::printf(" w=%ld ",where);
// INLINE FUNCTION BODIES moved to header file       double *p_value = pc_nDarray_rep->pd_nDdata + (size_t)where;
// INLINE FUNCTION BODIES moved to header file       return (*p_value);
...
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file   //##############################################################################
// INLINE FUNCTION BODIES moved to header file   // another overloading of operator() . . .  // overloaded for FOUR arguments
// INLINE FUNCTION BODIES moved to header file   inline double & nDarray::val(int first, int second,
// INLINE FUNCTION BODIES moved to header file                         int third, int fourth)
// INLINE FUNCTION BODIES moved to header file     {
// INLINE FUNCTION BODIES moved to header file //      ::printf("\n KORISTIM   Brzi_nDarray_val \n ");
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file       if(pc_nDarray_rep->nDarray_rank==0) return (*pc_nDarray_rep->pd_nDdata);
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file       long int where = first - 1;
// INLINE FUNCTION BODIES moved to header file                where = where*pc_nDarray_rep->dim[1]+second - 1;
// INLINE FUNCTION BODIES moved to header file                where = where*pc_nDarray_rep->dim[2]+third  - 1;
// INLINE FUNCTION BODIES moved to header file                where = where*pc_nDarray_rep->dim[3]+fourth - 1;
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file //     long int where = (((first-1)*pc_nDarray_rep->dim[1]+second-1)*pc_nDarray_rep->dim[2]+third-1)*pc_nDarray_rep->dim[3]+fourth-1;
// INLINE FUNCTION BODIES moved to header file //     long int where = (((first-1)*pc_nDarray_rep->dim[1] + second-1) * pc_nDarray_rep->dim[2]+ third-1)*pc_nDarray_rep->dim[3]+fourth-1);
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file      double *p_value = pc_nDarray_rep->pd_nDdata + (size_t)where;
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file      return (*p_value);
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file   //JB//JB    long int where = (((first-1)*pc_nDarray_rep->dim[1]+second-1)*pc_nDarray_rep->dim[2]+third-1)*pc_nDarray_rep->dim[3]+fourth-1;
// INLINE FUNCTION BODIES moved to header file //JB    double *p_value = pc_nDarray_rep->pd_nDdata + (size_t)where;
// INLINE FUNCTION BODIES moved to header file //JB// ..JB..    double *p_value = pc_nDarray_rep->pd_nDdata + (size_t)where;
// INLINE FUNCTION BODIES moved to header file //JB    return (*p_value);
// INLINE FUNCTION BODIES moved to header file //JB//    return (*pc_nDarray_rep->pd_nDdata + (size_t)(((first-1)*pc_nDarray_rep->dim[1]+second-1)*pc_nDarray_rep->dim[2]+third-1)*pc_nDarray_rep->dim[3]+fourth-1);
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file   //    return (*pc_nDarray_rep->pd_nDdata + (size_t)(((first-1)*pc_nDarray_rep->dim[1]+second-1)*pc_nDarray_rep->dim[2]+third-1)*pc_nDarray_rep->dim[3]+fourth-1);
// INLINE FUNCTION BODIES moved to header file   // BJ ..
// INLINE FUNCTION BODIES moved to header file   // BJ ..     long int where = first - 1;
// INLINE FUNCTION BODIES moved to header file   // BJ ..
// INLINE FUNCTION BODIES moved to header file   // BJ ..     if(pc_nDarray_rep->nDarray_rank==2)
// INLINE FUNCTION BODIES moved to header file   // BJ ..       {
// INLINE FUNCTION BODIES moved to header file   // BJ ..          where = where*pc_nDarray_rep->dim[1]+second - 1;
// INLINE FUNCTION BODIES moved to header file   // BJ ..       }
// INLINE FUNCTION BODIES moved to header file   // BJ ..
// INLINE FUNCTION BODIES moved to header file   // BJ ..     if(pc_nDarray_rep->nDarray_rank==3)
// INLINE FUNCTION BODIES moved to header file   // BJ ..       {
// INLINE FUNCTION BODIES moved to header file   // BJ ..         where = where*pc_nDarray_rep->dim[2]+third  - 1;
// INLINE FUNCTION BODIES moved to header file   // BJ ..       }
// INLINE FUNCTION BODIES moved to header file   // BJ ..
// INLINE FUNCTION BODIES moved to header file   // BJ ..     if(pc_nDarray_rep->nDarray_rank==4)
// INLINE FUNCTION BODIES moved to header file   // BJ ..       {
// INLINE FUNCTION BODIES moved to header file   // BJ ..         where = where*pc_nDarray_rep->dim[3]+fourth - 1;
// INLINE FUNCTION BODIES moved to header file   // BJ ..       }
// INLINE FUNCTION BODIES moved to header file   // BJ ..
// INLINE FUNCTION BODIES moved to header file   // BJ .. //::printf(" w=%ld ",where);
// INLINE FUNCTION BODIES moved to header file   // BJ ..     double *p_value = pc_nDarray_rep->pd_nDdata + (size_t)where;
// INLINE FUNCTION BODIES moved to header file   // BJ ..     return (*p_value);
// INLINE FUNCTION BODIES moved to header file     }
// INLINE FUNCTION BODIES moved to header file   // ..JB..
// INLINE FUNCTION BODIES moved to header file   // ..JB..
// INLINE FUNCTION BODIES moved to header file
...
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file // CVAL  CVAL  CVAL  CVAL
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file   // ..JB..//##############################################################################
// INLINE FUNCTION BODIES moved to header file   // ..JB..// another overloading of operator() . . .  // overloaded for ONE argument
// INLINE FUNCTION BODIES moved to header file   double & nDarray::cval(int first) const
// INLINE FUNCTION BODIES moved to header file     {
// INLINE FUNCTION BODIES moved to header file //      ::printf("\n KORISTIM   Brzi_nDarray_val \n ");
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file       long int where = first - 1;
...
// INLINE FUNCTION BODIES moved to header file   //##############################################################################
// INLINE FUNCTION BODIES moved to header file   // another overloading of operator() . . .  // overloaded for TWO arguments
// INLINE FUNCTION BODIES moved to header file   double & nDarray::cval(int first, int second) const
// INLINE FUNCTION BODIES moved to header file     {
// INLINE FUNCTION BODIES moved to header file //      ::printf("\n KORISTIM   Brzi_nDarray_val \n ");
// INLINE FUNCTION BODIES moved to header file       long int where = first - 1;
// INLINE FUNCTION BODIES moved to header file                where = where*pc_nDarray_rep->dim[1]+second - 1;
...
// INLINE FUNCTION BODIES moved to header file   //##############################################################################
// INLINE FUNCTION BODIES moved to header file   // another overloading of operator() . . .  // overloaded for THREE arguments
// INLINE FUNCTION BODIES moved to header file   double & nDarray::cval(int first, int second, int third ) const
// INLINE FUNCTION BODIES moved to header file     {
// INLINE FUNCTION BODIES moved to header file //      ::printf("\n KORISTIM   Brzi_nDarray_val \n ");
// INLINE FUNCTION BODIES moved to header file       long int where = first - 1;
// INLINE FUNCTION BODIES moved to header file                where = where*pc_nDarray_rep->dim[1]+second - 1;
...
// INLINE FUNCTION BODIES moved to header file   //##############################################################################
// INLINE FUNCTION BODIES moved to header file   // another overloading of operator() . . .  // overloaded for FOUR arguments
// INLINE FUNCTION BODIES moved to header file   double & nDarray::cval(int first, int second,
// INLINE FUNCTION BODIES moved to header file                         int third, int fourth)  const
// INLINE FUNCTION BODIES moved to header file     {
// INLINE FUNCTION BODIES moved to header file //      ::printf("\n KORISTIM   Brzi_nDarray_val \n ");
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file       if(pc_nDarray_rep->nDarray_rank==0) return (*pc_nDarray_rep->pd_nDdata);
...
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file
// INLINE FUNCTION BODIES moved to header file #endif
...
arg.pc_nDarray_rep->dim[%d]=%d\n",
    ...
    if ( this->pc_nDarray_rep->n > 1 )// see ARK in JOOP may/june '90
    {
        ...
        // out 19Nov2007// operator new
        // out 19Nov2007        int one_or0 = 0;
        // out 19Nov2007        if(!New_pc_nDarray_rep->nDarray_rank) one_or0 = 1;
        // out 19Nov2007
        // out 19Nov2007
        // out 19Nov2007        New_pc_nDarray_rep->dim = new int[New_pc_nDarray_rep->nDarray_rank+one_or0];
        // out 19Nov2007                                  // array for dimensions
        // out 19Nov2007


        //     NEW 19Nov2007
        New_pc_nDarray_rep->dim = new int[4];// array for dimensions
        ...
        // this way all the tensors have 4 dimensions and they are set to have
        // depth of 1 by default
        // moved to header file 19Nov2007        New_pc_nDarray_rep->dim[4] = {1,1,1,1};
        New_pc_nDarray_rep->dim[0] = this->pc_nDarray_rep->dim[0];
        New_pc_nDarray_rep->dim[1] = this->pc_nDarray_rep->dim[1];
        New_pc_nDarray_rep->dim[2] = this->pc_nDarray_rep->dim[2];
        New_pc_nDarray_rep->dim[3] = this->pc_nDarray_rep->dim[3];
...
nDarray  nDarray::operator+( double rval)
{
    ...
    for (int j = 0 ; j < pc_nDarray_rep->total_numb ; j++)
    {
        add.pc_nDarray_rep->pd_nDdata[j] += rval;
    }

    //optimized 19Nov2007 Boris Jeremic
    //      switch(pc_nDarray_rep->nDarray_rank)
    //        {
    //          case 0:
    //            {
...
nDarray  nDarray::operator-( double rval)
{
    ...
    for (int j = 0 ; j < pc_nDarray_rep->total_numb ; j++)
    {
        sub.pc_nDarray_rep->pd_nDdata[j] -= rval;
    }
